...
class AzureLLM(BaseLLM):
    ...
    def __init__(self, settings: Any, api_key: Optional[str] = None, endpoint: Optional[str] = None, deployment_name: Optional[str] = None, api_version: Optional[str] = None, **kwargs: Any) -> None:
    ...
    def chat(self, messages: List[Message], trace: Optional[Any] = None, **kwargs: Any) -> ChatResponse:
        ...
        # Make API call
        try:
            response_data = self._call_api(messages=api_messages, deployment=deployment, temperature=temperature, max_tokens=max_tokens)

            # Parse response
            content = response_data["choice"][0]["message"]["content"]
            usage = response_data.get("usage")

            return ChatResponse(content=content, model=response_data.get("model", deployment), usage=usage, raw_response=response_data)
    ...
    def _call_api(self, messages: List[Dict[str,str]], deployment: str, temperature: float, max_tokens:int) -> Dict[str, Any]:
        ...
        try:
            with httpx.Client(timeout=60.0) as client:
                ...
                error_detail = self._prase_error_response(response)
                raise AzureLLMError(
    ...
    def _prase_error_response(self, response: Any) -> str:
